...
# # Import existing class skillsets-api
# sys.path.append('//Users/lmasu/work/repos/skillsets/IaC/cdk/skillsets_api_pipeline')
# import skillsets_api
...
class PipelineStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        ARGOCD_CLUSTER_NAME = os.getenv(
            "ARGOCD_CLUSTER_NAME", "in-cluster"
        )  # Argocd cluster name where the applicaiton will be deployed
        ...
        REPOSITORY_URI = f"%s.dkr.ecr.{AWS_REGION}.amazonaws.com/iac-{APP_NAME}" % (
            AWS_ACCOUNT_ID
        )
        EKS_KUBECTL_ROLE_ARN = "arn:aws:iam::%s:role/EksCodeBuildKubectlRole" % (
            AWS_ACCOUNT_ID
        )
        APP_PATH = "../../../frontend"
        ...
        role = iam.Role(
            self,
            f"iac_{APP_NAME}_codebuild_role",
        ...
        ci_pipeline = codebuild.Project(
            self,
            f"iac_{APP_NAME}_codebuildproject_ci",
            ...
            secondary_sources=[
                codebuild.Source.code_commit(repository=k8s_repo, identifier="K8S")
            ],
            # Enable Docker layer and git caching
            cache=codebuild.Cache.local(
                codebuild.LocalCacheMode.DOCKER_LAYER, codebuild.LocalCacheMode.SOURCE
            ),
        )

        # Creates Codebuild Project for dev CD Pipeline
        cd_pipeline_dev = codebuild.Project(
            self,
            f"iac_{APP_NAME}_codebuildproject_cd_dev",
                    ...
                    "phases": {
                        ...
                        "build": {
                            "commands": [
                                ...
                                "kustomize edit set image KUSTOMIZE=$REPOSITORY_URI:$TAG",
        ...
        cd_pipeline_uat = codebuild.Project(
            self,
            f"iac_{APP_NAME}_codebuildproject_cd_uat",
            ...
            environment=codebuild.BuildEnvironment(
                environment_variables={
                    ...
                    "ENV": codebuild.BuildEnvironmentVariable(value="uat"),
                    "APP_NAME": codebuild.BuildEnvironmentVariable(value=APP_NAME),
                    "DEST_SERVER": codebuild.BuildEnvironmentVariable(
                        value=DEST_SERVER
                    ),
        ...
        cd_pipeline_auto_test = codebuild.Project(
            self,
            f"iac_{APP_NAME}_codebuildproject_auto_test",
            ...
            environment=codebuild.BuildEnvironment(
                environment_variables={
                    ...
                    "ENV": codebuild.BuildEnvironmentVariable(value="uat"),
                    "APP_NAME": codebuild.BuildEnvironmentVariable(value=APP_NAME),
                    "K6_DURATION": codebuild.BuildEnvironmentVariable(value="1m"),
                    "K6_VUs": codebuild.BuildEnvironmentVariable(value="250"),
                },
                # privileged=True,
        ...
        pipeline = codepipeline.Pipeline(
            self,
            f"iac_{APP_NAME}_pipeline",
            pipeline_name=f"IaC_{APP_NAME}_Pipeline",
            stages=[
                codepipeline.StageProps(
                    stage_name="Source", actions=[app_source_action]
                ),
                codepipeline.StageProps(stage_name="Build", actions=[ci_build_action]),
                codepipeline.StageProps(
                    stage_name="DEV", actions=[cd_build_action_dev]
                ),
                codepipeline.StageProps(
                    stage_name="UAT",
        ...
        # slack_workspace_id (str) – The ID of the Slack workspace authorized with AWS Chatbot.
        ...
        # Import existing slack chanel
        slack = chatbot.SlackChannelConfiguration.from_slack_channel_configuration_arn(self,"ImportedSlackChannel",cdk.Fn.import_value("iac-slack"))
        
        # rule1 = ci_pipeline.notify_on_build_succeeded("NotifyOnBuildSucceeded",slack )
        rule1 = cd_pipeline_dev.notify_on(
            "NotifyOnAll",
            target=slack,
            events=[
        ...
        rule3 = cd_pipeline_uat.notify_on(
            "NotifyOnAll",
            target=slack,
            events=[
                codebuild.ProjectNotificationEvents.BUILD_FAILED,
        ...
        rule4 = cd_pipeline_auto_test.notify_on(
            "NotifyOnAll",
            target=slack,
            events=[
                codebuild.ProjectNotificationEvents.BUILD_FAILED,
        ...
        cdk.CfnOutput(
            self,
            f"iac-{APP_NAME}-app-repository",
            export_name=f"iac-{APP_NAME}-app-repository",
            value=app_repo.repository_clone_url_http,
        )
        cdk.CfnOutput(
            self,
            f"iac-{APP_NAME}-k8s-repository",
            export_name=f"iac-{APP_NAME}-k8s-repository",
            value=k8s_repo.repository_clone_url_http,
        )

app = cdk.App()
_env=cdk.Environment(account=os.getenv("AWS_ACCOUNT_ID"), region=os.getenv("AWS_REGION"))
PipelineStack(app,"SkilllsetsUIPipelineStack",env=_env)
app.synth()

# aws ecr delete-repository --repository-name iac-skillsets-ui --force || true && printf y | cdk destroy --require-approval never && cdk deploy --require-approval never
